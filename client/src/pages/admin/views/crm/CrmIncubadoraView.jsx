 ...
 *
 * Agencia: promotoría completa (leaderboard + nuevos).
 * Asesor: el COACH DE VENTAS es la sección (chat a pantalla completa, siempre
 * Haiku en el server); los consejos/hábitos viven en una pestaña secundaria.
 */
...
const SUGERENCIAS_COACH_AGENCIA = [
  '¿Qué asesor necesita mi ayuda hoy y por qué?',
  'Dame un guion para que el equipo pida referidos',
  '¿Cómo entreno al equipo contra "lo tengo que pensar"?',
  '¿Qué cobros y rehabilitaciones empujo hoy?',
];
...
export default function CrmIncubadoraView() {
  ...
  const [tab, setTab] = useState('coach'); // asesor: coach | consejos
  ...
  const consejosHoy = d && esAsesor && d.yo && (
    <div className="crm-chart-card" style={{ marginBottom: 24, borderTop: `3px solid ${C.gold}` }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        ...
        <div style={{ fontSize: 13 }}>🤝 ¿Dudas de cómo vas o qué sigue? Pregúntale a tu <b>Coach</b> (la otra pestaña de esta sección) — conoce tus números al día.</div>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="crm-toolbar">
        <div>
          <h2 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}><GraduationCap size={20} color={C.accent} /> {esAsesor ? 'Tu coach de ventas' : 'Incubadora de vendedores'}</h2>
          <p className="sub" style={{ margin: '2px 0 0' }}>
            {esAsesor
              ? 'Objeciones, guiones, referidos y cierres — con tus números reales en mente.'
              : 'Coach del equipo con los números vivos + el playbook de los mejores.'}
          </p>
        </div>
      ...
      {/* ── El COACH ES la sección (asesor y agencia); lo demás en pestaña ── */}
      {d && (
        <>
          <style>{chatCSS}</style>
          <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
            <button className={`crm-dtab${tab === 'coach' ? ' active' : ''}`} onClick={() => setTab('coach')}>🧠 Coach de ventas</button>
            <button className={`crm-dtab${tab === 'consejos' ? ' active' : ''}`} onClick={() => setTab('consejos')}>{esAsesor ? '💡 Consejos y hábitos' : '📊 Playbook y tabla'}</button>
          </div>

          {tab === 'coach' && (
            <div className="crm-chart-card" style={{ padding: 0, overflow: 'hidden', borderTop: `3px solid ${C.gold}`, display: 'flex', flexDirection: 'column' }}>
              <div style={{ background: `linear-gradient(135deg,${C.navy},#0A2A66)`, color: '#fff', padding: '14px 18px' }}>
                <b style={{ fontSize: 15, display: 'flex', alignItems: 'center', gap: 8 }}><Sparkles size={17} color={C.gold} /> {esAsesor ? 'Tu coach de ventas' : 'Coach de ventas de la promotoría'}</b>
                <div style={{ fontSize: 12, opacity: .8, marginTop: 2 }}>
                  {esAsesor
                    ...
                    : 'Guiones y técnicas para entrenar al equipo, y qué empujar hoy — con los números vivos de toda la promotoría.'}
                </div>
              </div>
              <ChatPanel
                modo="coach"
                alto="calc(100vh - 305px)"
                sugerencias={esAsesor ? SUGERENCIAS_COACH : SUGERENCIAS_COACH_AGENCIA}
                intro={esAsesor
                  ? '¡Hola, coach al habla! 🧠 Cuéntame con qué cliente u objeción estás batallando, o pídeme un guion — te doy una técnica concreta lista para usar, pensada para tu cartera y tu siguiente bono.'
                  : '¡Hola, coach al habla! 🧠 Pregúntame cómo entrenar al equipo, qué guiones darles o qué empujar hoy — respondo con los números reales de cada asesor.'}
              />
            </div>
          )}

          {tab === 'consejos' && esAsesor && (
            <>
              {benchmark}
              {playbook}
              {consejosHoy}
      ...
      {/* ── AGENCIA, pestaña Playbook: vista completa como estaba ── */}
      {d && !esAsesor && tab === 'consejos' && (
        <>
          {benchmark}
          {playbook}
          {d.leaderboard && (
