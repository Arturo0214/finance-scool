...
export const api = {
  ...
  crmChat: (messages, modo) => request('/crm/chat', { method: 'POST', body: JSON.stringify({ messages, ...(modo ? { modo } : {}) }) }),
  ...
  crmSemillasColCreate: (data) => request('/crm/semillas/columnas', { method: 'POST', body: JSON.stringify(data) }),
  crmSemillasColUpdate: (id, data) => request(`/crm/semillas/columnas/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  crmSemillasColDelete: (id) => request(`/crm/semillas/columnas/${id}`, { method: 'DELETE' }),
  crmSemillasLeads: (q) => request(`/crm/semillas/leads${q ? `?q=${encodeURIComponent(q)}` : ''}`),
  crmSemillasLead: (id) => request(`/crm/semillas/leads/${id}`),
  crmSemillasCelda: (id, key, valor) => request(`/crm/semillas/leads/${id}/celda`, { method: 'PATCH', body: JSON.stringify({ key, valor }) }),
  crmSemillasServicioAdd: (id, data) => request(`/crm/semillas/leads/${id}/servicios`, { method: 'POST', body: JSON.stringify(data) }),
  crmSemillasServicioUpdate: (id, data) => request(`/crm/semillas/servicios/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  crmSemillasSeguimiento: (id, data) => request(`/crm/semillas/leads/${id}/seguimientos`, { method: 'POST', body: JSON.stringify(data) }),
  crmCampanas: () => request('/crm/campanas'),
  crmCampanaAvance: (slug, clave) => request(`/crm/campanas/${encodeURIComponent(slug)}/avance${clave ? `?clave=${encodeURIComponent(clave)}` : ''}`),
  crmFirefliesStatus: () => request('/crm/fireflies/status'),
  crmFirefliesTranscripts: (limit = 25) => request(`/crm/fireflies/transcripts?limit=${limit}`),
  crmFirefliesTranscript: (id) => request(`/crm/fireflies/transcript/${encodeURIComponent(id)}`),
  crmFirefliesImport: (clientId, transcriptId) => request(`/crm/clients/${clientId}/fireflies/import`, { method: 'POST', body: JSON.stringify({ transcript_id: transcriptId }) }),
